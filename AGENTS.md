...
A self-service temporary local admin access system for macOS endpoints managed by Iru MDM. Users request access via a Iru Self Service script, an IT admin approves in Slack, the device is elevated for 30 minutes, and a sudo audit log is shipped back to Slack on expiration.

There are three distinct layers:
1. **Backend** — AWS Lambda functions + API Gateway + DynamoDB + EventBridge (in `lambdas/` and `infrastructure/`)
2. **Device scripts** — bash scripts that run on macOS endpoints via Iru Library Items (in `scripts/`)
3. **MDM configuration** — Iru tags, SAP Privileges profile, and Self Service presentation
...
3. **Get a Iru API token** — `docs/iru-setup.md` step 5-6.
...
7. **Configure Iru** — create tags, upload Privileges profile, upload scripts. See `docs/iru-setup.md`.
...
### Iru script changes
Scripts are managed with [kst](https://github.com/iru-inc/kst). After editing files in `scripts/`:
...
### No `blankPush`
The codebase has no `blankPush` calls. `blankPush` only triggers an MDM check-in, not Library Item processing — it cannot run custom scripts. Device-side `iru run --reset-daily` is the only mechanism for picking up tag changes.
...
### Mutex lock
A file-based lock at `/var/run/iru-run.lock` prevents concurrent `iru run` calls from the approval monitor, network monitor, and expiration runner daemons. The lock file stores the holding PID. Always use `acquire_iru_run_lock` / `release_iru_run_lock` before calling `iru run --reset-daily`.

### Post-run verification with retry
After `iru run --reset-daily`, daemons verify the expected state change occurred (user is admin, or user is no longer admin) via `dseditgroup`. If not confirmed, one retry fires after 120 seconds. This accommodates Iru backend tag propagation latency (typically 30–120s).
...
Update the `elevationDurationMinutes` constant in `lambdas/handleElevationStart/index.js`. Also update the `DockToggleTimeout` in `infrastructure/privileges-config.mobileconfig` to match. Also update the `ExpirationInterval` in the Privileges MDM profile in Iru.
...
- **Do not add `iru run` inside `elevation-start.sh`** — it causes a deadlock (the script runs inside a iru run).
...
- **Do not commit `kst-repo/`** — it contains Iru-specific sync state and is in `.gitignore`.
